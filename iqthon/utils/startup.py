...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/88b0307eda38438f2eae4.jpg", caption="♛ ⦙ تـمّ  اعـادة تشـغيل\n رسثون  ✓  :  [ 7.6 ] .\n\n♛ ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n♛ ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n♛ ⦙ القناة الرسمية رسثون  : @RS_SY\n♛ ⦙ فارات سورس رسثون  :@RS_HB1 \n♛ ⦙ كلايش رسثون :  @RS_ZO\n ♛ ⦙التحديثات والاضافات :  @RS_AZ\n",                buttons=[(Button.url("مطور رسثون الرسمي", "https://t.me/I_1N_0"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        iqphoto1 = await iqthon.upload_file(file="SQL/extras/iqthon11.jpg")
        _, groupid = await create_supergroup(            "تخزين رسثون  العام", iqthon, Config.TG_BOT_USERNAME, descript  , iqphoto1      )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        iqphoto2 = await iqthon.upload_file(file="SQL/extras/iqthon11.jpg")
        _, groupid = await create_supergroup(            "تخزين رسثون  الخاص", iqthon, Config.TG_BOT_USERNAME, descript   ,  iqphoto2    )
